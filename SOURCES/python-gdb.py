...
class Frame(object):
    ...
    def is_waiting_for_lock(self):
        '''Is this frame waiting for a lock?'''
        framename = self._gdbframe.name()
        if framename:
            return framename.startswith('PyThread_acquire_lock')
    ...
    def print_summary(self):
        if self.is_evalframeex():
            ...
            if pyop:
                ...
                py_co = self.get_py_co()
                if py_co:
                    sys.stdout.write('#%i file %s, in %s\n'
                                     % (self.get_index(), 
                                        py_co.field('co_filename'),
                                        py_co.field('co_name')))
                else:
